...
export async function getHomeSchemas(locale: string): Promise<Schema[]> {
  ...
  const organizationSchema = createSchema("Organization", {
    ...
    legalName: businessConfig.company.legalName,
    url: baseUrl,
    logo: {
      "@type": "ImageObject",
      url: `${baseUrl}${seoConfig.defaultOgImage}`,
    ...
    foundingDate: businessConfig.company.foundingDate,
    founders: [
      {
        "@type": "Person",
        name: businessConfig.founder.name,
        jobTitle: businessConfig.founder.jobTitle,
      },
    ],
    address: buildPostalAddressSchema(),
    contactPoint: [buildContactPointSchema()],
    sameAs: buildSameAsArray(),
  });
  ...
  const localBusinessSchema = createSchema("LocalBusiness", {
    ...
    name: businessConfig.company.displayName,
    alternateName: businessConfig.company.legalName,
    description: jsonLdT("organizationDescription"),
    slogan: getSlogan(locale),
    image: `${baseUrl}${seoConfig.defaultOgImage}`,
    logo: `${baseUrl}/icon1.png`,
    telephone: businessConfig.contact.phoneRaw,
    email: businessConfig.contact.email,
    address: buildPostalAddressSchema(),
    areaServed: [
      ...businessConfig.serviceArea.primary,
      businessConfig.serviceArea.region,
      businessConfig.serviceArea.country,
    ],
    geo: buildGeoCoordinatesSchema(),
    url: baseUrl,
    priceRange: businessConfig.payment.priceRange,
    currenciesAccepted: businessConfig.payment.currenciesAccepted.join(", "),
    paymentAccepted: businessConfig.payment.paymentAccepted.join(", "),
    openingHoursSpecification: buildOpeningHoursSchema(),
    hasOfferCatalog: {
      "@type": "OfferCatalog",
      name:
        locale === "cs"
